  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: 48, height: 48, border: "3px solid #D8F3DC", borderTopColor: "#40916C", borderRadius: "50%", animation: "spin 1s linear infinite", margin: "0 auto 1rem" }} />
      ...
      <div style={{ background: "linear-gradient(135deg, #2D6A4F 0%, #40916C 60%, #52b788 100%)", borderRadius: 24, padding: "2rem 2.5rem", marginBottom: "2rem", position: "relative", overflow: "hidden", boxShadow: "0 20px 60px rgba(45,106,79,0.3)" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", gap: "2rem", marginTop: "1.75rem", flexWrap: "wrap" }}>
            {[{ label: "Active children", value: active + " enrolled" }, { label: "Reports", value: reports.length + " written" }, { label: "Total referrals", value: referrals.length + " received" }].map(s => (
              <div key={s.label}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: "1.25rem", marginBottom: "1.25rem" }}>
          ...
          <div style={{ padding: "0.5rem 0" }}>
            {referrals.length === 0 ? (
              <div style={{ textAlign: "center", padding: "3rem 1.5rem" }}>
                <FileText size={32} style={{ color: G, margin: "0 auto 1rem", display: "block" }} />
                <p style={{ fontWeight: 600, color: "#64748b", fontSize: "0.9rem" }}>No referrals yet</p>
              </div>
            ) : referrals.slice(0, 5).map(r => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.85rem 1.5rem", borderBottom: "1px solid #f8fafc" }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: GP, display: "flex", alignItems: "center", justifyContent: "center", color: G, flexShrink: 0, fontWeight: 700 }}>{(r.childName || r.referrerName || "?")[0].toUpperCase()}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, color: "#0f172a", fontSize: "0.875rem" }}>{r.childName || "Unknown child"}</div>
        ...
        <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #f1f5f9", boxShadow: "0 2px 12px rgba(0,0,0,0.04)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "0.5rem 0" }}>
            {children.length === 0 ? (
              <div style={{ textAlign: "center", padding: "3rem 1.5rem" }}>
                <Users size={32} style={{ color: "#3b82f6", margin: "0 auto 1rem", display: "block" }} />
              ...
              <Link key={c.id} href={"/admin/children/" + c.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.85rem 1.5rem", borderBottom: "1px solid #f8fafc", textDecoration: "none" }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "#dbeafe", display: "flex", alignItems: "center", justifyContent: "center", color: "#3b82f6", flexShrink: 0, fontWeight: 700 }}>{c.name[0].toUpperCase()}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, color: "#0f172a", fontSize: "0.875rem" }}>{c.name}</div>
                  <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>Age {c.age}</div>
                </div>
                <div style={{ display: "flex", gap: "4px" }}>
                  {[c.progress?.christianLife, c.progress?.education, c.progress?.personalLife].map((p, i) => (
                    <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: p && p !== "not_started" ? G : "#e2e8f0" }} />
                  ))}
                </div>
              </Link>
            ))}
            ...
            return (
              <div key={p.label} style={{ background: BG, borderRadius: 16, padding: "1.25rem", border: "1px solid rgba(64,145,108,0.1)" }}>
                ...
                <div style={{ height: 6, borderRadius: 100, background: "#e2e8f0", overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: 100, background: p.color, width: pct + "%", transition: "width 1s ease" }} />
                </div>
