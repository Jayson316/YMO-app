...
// ─── CHILDREN ────────────────────────────────────────────────
export async function addChild(data: Omit<Child, "id" | "reports">) {
  return addDoc(collection(db, "children"), { ...data, reports: [] });
}
...
export async function getChildReports(childId: string): Promise<Report[]> {
  const snap = await getDocs(
    query(collection(db, "reports"), where("childId", "==", childId), orderBy("date", "desc"))
  );
  return snap.docs.map((d) => ({ id: d.id, ...d.data() } as Report));
}
